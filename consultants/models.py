...
def train_smart_money_model(trades, voting_scores=None):
    ...
    def features(data, trade):
        ...
        # ✅ الربط مع عمود البوت الحقيقي whale_confidence
        whale = float(trade.get('whale_confidence', 0) or 0)
        inflow = data.get('exchange_inflow', 0) or data.get('whale_activity', 0)
        # 🆕 إضافة الميزات الناقصة
        whale_wallet_changes = float(trade.get('whale_wallet_changes', 0))
        institutional_accumulation = float(trade.get('institutional_accumulation', 0))
        smart_money_ratio = float(trade.get('smart_money_ratio', 0))
        exchange_whale_flows = float(trade.get('exchange_whale_flows', 0))
        base.extend([whale, inflow, whale_wallet_changes, institutional_accumulation, smart_money_ratio, exchange_whale_flows, scores.get('tp_accuracy', 0.5), scores.get('sell_accuracy', 0.5)])
        return base

    names = SAFE_NAMES + ['whale_activity', 'exchange_inflow', 'whale_wallet_changes', 'institutional_accumulation', 'smart_money_ratio', 'exchange_whale_flows', 'tp_accuracy', 'sell_accuracy']
...
def train_risk_model(trades, voting_scores=None):
    ...
    def features(data, trade):
        ...
        atr = data.get('atr', 0)
        # 🆕 إضافة الميزات الناقصة
        volatility_risk_score = float(trade.get('volatility_risk_score', 0))
        correlation_risk = float(trade.get('correlation_risk', 0))
        gap_risk_score = float(trade.get('gap_risk_score', 0))
        black_swan_probability = float(trade.get('black_swan_probability', 0))
        behavioral_risk = float(trade.get('behavioral_risk', 0))
        systemic_risk = float(trade.get('systemic_risk', 0))
        base.extend([rsi, atr, volatility_risk_score, correlation_risk, gap_risk_score, black_swan_probability, behavioral_risk, systemic_risk, scores.get('tp_accuracy', 0.5), scores.get('sell_accuracy', 0.5)])
        return base

    names = SAFE_NAMES + ['risk_rsi', 'risk_atr', 'volatility_risk_score', 'correlation_risk', 'gap_risk_score', 'black_swan_probability', 'behavioral_risk', 'systemic_risk', 'tp_accuracy', 'sell_accuracy']
...
def train_anomaly_model(trades, voting_scores=None):
    ...
    def features(data, trade):
        ...
        # 🆕 إضافة الميزات الناقصة
        statistical_outliers = float(trade.get('statistical_outliers', 0))
        pattern_anomalies = float(trade.get('pattern_anomalies', 0))
        behavioral_anomalies = float(trade.get('behavioral_anomalies', 0))
        volume_anomalies = float(trade.get('volume_anomalies', 0))
        base.extend([anomaly, statistical_outliers, pattern_anomalies, behavioral_anomalies, volume_anomalies, scores.get('tp_accuracy', 0.5), scores.get('sell_accuracy', 0.5)])
        return base

    names = SAFE_NAMES + ['anomaly_score', 'statistical_outliers', 'pattern_anomalies', 'behavioral_anomalies', 'volume_anomalies', 'tp_accuracy', 'sell_accuracy']
...
def train_exit_model(trades, voting_scores=None):
    ...
    def features(data, trade):
        ...
        # 🆕 إضافة الميزات الناقصة
        profit_optimization_score = float(trade.get('profit_optimization_score', 0))
        time_decay_signals = float(trade.get('time_decay_signals', 0))
        opportunity_cost_exits = float(trade.get('opportunity_cost_exits', 0))
        market_condition_exits = float(trade.get('market_condition_exits', 0))
        base.extend([hours, profit_optimization_score, time_decay_signals, opportunity_cost_exits, market_condition_exits, scores.get('tp_accuracy', 0.5), scores.get('sell_accuracy', 0.5)])
        return base

    names = SAFE_NAMES + ['hours_held', 'profit_optimization_score', 'time_decay_signals', 'opportunity_cost_exits', 'market_condition_exits', 'tp_accuracy', 'sell_accuracy']

    def label(t):
        # ✅ إصلاح: الـ label من profit فقط — لا يعتمد على trade_quality
        profit = float(t.get('profit_percent', 0))
        return 1 if profit >= 1.0 else 0  # كان 10.0 - تخفيض لتجنب فشل التدريب
...
def train_pattern_model(trades, voting_scores=None):
    ...
    def features(data, trade):
        ...
        # 🆕 إضافة الميزات الناقصة
        harmonic_patterns_score = float(trade.get('harmonic_patterns_score', 0))
        elliott_wave_signals = float(trade.get('elliott_wave_signals', 0))
        fractal_patterns = float(trade.get('fractal_patterns', 0))
        cycle_patterns = float(trade.get('cycle_patterns', 0))
        momentum_patterns = float(trade.get('momentum_patterns', 0))
        base.extend([momentum, harmonic_patterns_score, elliott_wave_signals, fractal_patterns, cycle_patterns, momentum_patterns, scores.get('tp_accuracy', 0.5), scores.get('sell_accuracy', 0.5)])
        return base

    names = SAFE_NAMES + ['pattern_momentum', 'harmonic_patterns_score', 'elliott_wave_signals', 'fractal_patterns', 'cycle_patterns', 'momentum_patterns', 'tp_accuracy', 'sell_accuracy']
...
def train_liquidity_model(trades, voting_scores=None):
    ...
    for trade in trades:
        try:
            ...
            liq = data.get('liquidity', {})
            # محاولة جلب السيولة من الـ JSON أو عمود الداتابيز أو استخدام قيمة محايدة
            liquidity_score = liq.get('liquidity_score') or trade.get('liquidity_score')
            if liquidity_score is None:
                liquidity_score = 50  # قيمة محايدة
            
            # إذا كانت البيانات مفقودة تماماً، لا نتخطى الصفقة بل نضع قيم افتراضية
            if not liq:
                depth_ratio = 1.0
                price_impact = 0.5
            ...
            features_list.append([
                ...
                # 🆕 الميزات المتقدمة من الداتابيز (5)
    ...
    print(f"Liquidity Model: Accuracy {acc*100:.2f}% | Learns from {len(features_list)} trades with {len(names)} features")
    return model, acc
...
def train_chart_cnn_model(trades, voting_scores=None):
    ...
    names = SAFE_NAMES + ['bullish_chart', 'bearish_chart', 'neutral_chart', 'attention_mechanism_score', 'multi_scale_features', 'temporal_features', 'tp_accuracy', 'sell_accuracy']
...
def train_volume_prediction_model(trades, voting_scores=None):
    ...
    def features(data, trade):
        ...
        # ✅ إزالة price_change وأي مشتقات منها لمنع تسريب النتيجة للموديل
        # 🆕 إضافة الميزات الناقصة
        volume_trend_strength = float(trade.get('volume_trend_strength', 0))
        volume_seasonality = float(trade.get('volume_seasonality', 0))
        volume_correlation = float(trade.get('volume_correlation', 0))
        ...
        return [
            ...
            data.get('price_change_24h_old', 0), # استخدام بيانات قديمة جداً فقط إذا لزم الأمر
