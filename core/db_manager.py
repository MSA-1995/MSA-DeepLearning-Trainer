...
class DatabaseManager:
    ...
    def check_training_trigger(self, min_new_trades=300):  # الإعداد الأمثل - توازن بين الجودة والسرعة
        """
        ...
        # المرة الأولى: لا يوجد توقيت (تدريب على كل شيء)
        if latest_ts is None:
            print("🆕 First time training detected. Loading all historical trades.")
            return True, None
            
        # المرات التالية: فحص عدد الصفقات الجديدة
        conn = self._get_conn()
        try:
            with conn.cursor() as cursor:
                cursor.execute("""
        ...
        for attempt in range(retry):
            ...
            try:
                ...
                with conn.cursor() as cursor:
                    ...
                    """)
                    # ✅ تنظيف المكررات وفرض قيد الـ UNIQUE لضمان عمل التحديث الذكي
                    cursor.execute("DELETE FROM dl_models_v2 a USING dl_models_v2 b WHERE a.id < b.id AND a.model_name = b.model_name;")
                    cursor.execute("ALTER TABLE dl_models_v2 DROP CONSTRAINT IF EXISTS dl_models_v2_model_name_key;")
                    cursor.execute("ALTER TABLE dl_models_v2 ADD CONSTRAINT dl_models_v2_model_name_key UNIQUE (model_name);")
                conn.commit()
                ...
                for model_name, model_obj in models.items():
                    ...
                    # ✅ منع استبدال الموديلات بموديلات ضعيفة جداً
                    if accuracy < 0.40:
                        # print(f"  ⚠️ {model_name}: skipping SAVE (accuracy {accuracy*100:.1f}% too low to replace injected model)")
                        continue

                    # ✅ الشرط الأساسي: الحفظ فقط إذا كانت الثقة (Accuracy) أعلى حصراً
                    ...
                    try:
                        ...
                        pass  # If error, assume 0

                    # إذا كانت الدقة الجديدة أقل أو تساوي القديمة، نتجاهل الحفظ
                    if accuracy <= existing_accuracy:
                        print(f"  ⏸️ {model_name}: skipped (New {accuracy*100:.1f}% <= Existing {existing_accuracy*100:.1f}%)")
                        # ✅ تحديث توقيت التدريب فقط لتعليم الصفقات كـ "تمت معالجتها" لمنع التكرار
                        try:
                            with conn.cursor() as cursor:
                    ...
                    try:
                        ...
                        with conn.cursor() as cursor:
                            ...
                            cursor.execute("""
                                ...
                                DO UPDATE SET accuracy = EXCLUDED.accuracy, model_data = EXCLUDED.model_data, trained_at = NOW();
    ...
    def _calculate_symbol_stats(self, symbol, trades):
        ...
        courage_boost = min(win_rate * 20, 15) if win_rate >= 0.1 else 0
        ...
        pattern_score = min((total / 10) * (win_rate * 2), 20) if total >= 1 else 0
