...
class DeepLearningTrainerLightGBM:
    ...
    def train_all_models(self):
        ...
        # Train meta_learner separately
        try:
            if 'meta_learner' in missing_models:
                # Missing - train on ALL trades
                if trades_all and len(trades_all) > 0:
                    print(f"\n👑🧠 Meta-Learner: training on ALL trades ({len(trades_all)})...")
                    meta_result = train_meta_learner_model(self.db, trained_consultants, voting_scores, since_timestamp=None)
                    if meta_result:
                        self.models['meta_learner'], results['meta_learner_accuracy'] = meta_result
                        print(f"  ✅ Meta-Learner: Accuracy {results['meta_learner_accuracy']*100:.2f}%")
                else:
                    print(f"\n👑🧠 Meta-Learner: skipping (no data)")
            else:
                # Existing - train on NEW trades
                if trades_new and len(trades_new) > 0:
                    print(f"\n👑🧠 Meta-Learner: training on NEW trades ({len(trades_new)})...")
                    oldest_timestamp = self.db.get_oldest_model_timestamp()
                    meta_result = train_meta_learner_model(self.db, trained_consultants, voting_scores, since_timestamp=oldest_timestamp)
                    if meta_result:
                        self.models['meta_learner'], results['meta_learner_accuracy'] = meta_result
                        print(f"  ✅ Meta-Learner: Accuracy {results['meta_learner_accuracy']*100:.2f}%")
                else:
