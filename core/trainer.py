...
# استيراد chart_cnn من مجلد models بدلاً من consultants
from models.chart_cnn_model import train_chart_cnn_model
...
from models.candle_expert_model import train_candle_expert_model
from models.meta_trading import train_meta_trading
# New models from TradingBot-AI
from models.smart_money_model import train_smart_money_model
from models.risk_manager_model import train_risk_model
from models.anomaly_detector_model import train_anomaly_model
from models.exit_strategy_model import train_exit_strategy_model
from models.pattern_recognition_model import train_pattern_recognition_model
from models.liquidity_analyzer_model import train_liquidity_model
...
class DeepLearningTrainerLightGBM:
    ...
    def train_all_models(self):
        ...
        # ✅ استخدام التوقيت الموحد من نظام الـ Trigger (الإعداد الأمثل: 300)
        should_train, since_ts = self.db.check_training_trigger(300)  # كان 100
        missing_models = self.db.get_missing_models()
        
        if not should_train and not missing_models:
            print(f"⏭️ Skipping cycle: No enough new trades (+300) since last attempt.")
            return False
        ...
        try:
            if 'meta_trading' in missing_models:
                ...
                if trades_all and len(trades_all) > 0:
                    ...
                    meta_result = train_meta_trading(trades_all, voting_scores, since_timestamp=None, db_manager=self.db, trained_consultants=self.trained_consultants)
                    if meta_result:
                ...
                if trades_new and len(trades_new) > 0:
                    ...
                    meta_result = train_meta_trading(trades_new, voting_scores, since_timestamp=oldest_timestamp, db_manager=self.db, trained_consultants=self.trained_consultants)
                    if meta_result:
                         self.models['meta_trading'], results['meta_trading_accuracy'] = meta_result
                         print(f"  ✅ Meta-Model: Accuracy {results['meta_trading_accuracy']*100:.2f}%")
                else:
